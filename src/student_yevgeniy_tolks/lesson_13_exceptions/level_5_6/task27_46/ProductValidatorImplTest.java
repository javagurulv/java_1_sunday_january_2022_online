...
class ProductValidatorImplTest {
    ...
    public static void main(String[] args) {
        ...
        test.Rule9TitleIsNullAndPriceIsZeroTest();
        test.Rule10TitleIsEmptyAndPriceIsZeroTest();
        test.Rule11WrongTitleAndDescriptionFormatTest();
        test.Rule12LengthOfTitleAndDescriptionOutOFRangeTest();
        test.rule13AllThreeRulesFailedTest();
    }
    ...
    public void Rule9TitleIsNullAndPriceIsZeroTest() {
        Product product = new Product(null, BigDecimal.ZERO, "Some description");
        List<ValidationException> exceptions = productValidator.validate(product);
        ValidationException rule1 = new ValidationException("RULE-1", "Title can not be Empty", "title");
        ValidationException rule6 = new ValidationException("RULE-6", "Price can not be Zero", "price");
        checkResults(exceptions.contains(rule1) && exceptions.contains(rule6), "rule9");
    }

    public void Rule10TitleIsEmptyAndPriceIsZeroTest() {
        Product product = new Product("", BigDecimal.ZERO, "Some description");
        List<ValidationException> exceptions = productValidator.validate(product);
        ValidationException rule1 = new ValidationException("RULE-1", "Title can not be Empty", "title");
        ValidationException rule6 = new ValidationException("RULE-6", "Price can not be Zero", "price");
        checkResults(exceptions.contains(rule1) && exceptions.contains(rule6), "rule10");
    }

    public void Rule11WrongTitleAndDescriptionFormatTest() {
        ...
        ValidationException rule8 = new ValidationException("RULE-8", "Description should have english " +
                "letters and numbers", "description");
        checkResults(exceptions.contains(rule4) && exceptions.contains(rule8), "rule11");
    }

    public void Rule12LengthOfTitleAndDescriptionOutOFRangeTest() {
        String productTitleLength = String.valueOf(new char[101]);
        String descriptionLength = String.valueOf(new char[2001]);
        ...
        ValidationException rule7 = new ValidationException("RULE-7", "To long product description content", "description");
        ValidationException rule3 = new ValidationException("RULE-3", "Title can not be longer than 100 symbols", "title");
        checkResults(exceptions.contains(rule3) && exceptions.contains(rule7), "rule12");
    }

    public void rule13AllThreeRulesFailedTest() {
        ...
        ValidationException rule8 = new ValidationException("RULE-8", "Description should have english " +
                "letters and numbers", "description");
        checkResults(
                exceptions.contains(rule1)
                        && exceptions.contains(rule6)
                        && exceptions.contains(rule8), "rule13");
    }
